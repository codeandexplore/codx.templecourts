...
Live review session where a Teacher walks a Student through lesson questions in tree-traversal order. Teacher marks answers as reviewed (live-only), flags unanswered questions, and the session supports multi-session resumption. Real-time sync via SignalR.
