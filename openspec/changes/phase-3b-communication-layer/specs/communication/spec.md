# Teacher Check Questions

## Purpose

Reusable bank of check-understanding prompts per Teacher, keyed on stable `question_key`. Orphan status computed at read-time — not stored. Survives lesson version bumps.

## ADDED Requirements

### Requirement: Teacher can create a check-understanding question

The system SHALL allow a Teacher to create a check question linked to a `question_key`. The question SHALL be scoped to the creating Teacher.

#### Scenario: Create check question
- **WHEN** a Teacher sends `POST /api/check-questions` with `{ "questionKey": "...", "noteText": "..." }`
- **THEN** a TeacherCheckQuestion is created and returned

### Requirement: Orphan status is computed at read-time

When reading check questions, the system SHALL flag any whose `question_key` is not present in the lesson's current published version's question set as orphaned.

#### Scenario: Check question for removed question
- **WHEN** a lesson version retires a question and a Teacher reads their check questions
- **THEN** the retired question's check question is flagged as orphaned

### Requirement: Teacher can delete their own check questions

The system SHALL allow a Teacher to delete their own check questions.

#### Scenario: Delete check question
- **WHEN** a Teacher sends `DELETE /api/check-questions/{id}`
- **THEN** the check question is removed
